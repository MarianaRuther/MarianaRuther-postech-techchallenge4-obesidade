...
Este módulo implementa visualizações e análises estatísticas
sobre o dataset de obesidade.
"""
...
from config import (
    DATA_FILE, OBESITY_MAP, COLORS_PT, GENDER_MAP,
    STREAMLIT_CONFIG, BMI_THRESHOLDS
)
...
# Configuração da página (só executa uma vez - compatível com app.py)
if 'page_configured' not in st.session_state:
    st.set_page_config(
        page_title=STREAMLIT_CONFIG['page_title_dashboard'],
        page_icon=STREAMLIT_CONFIG['page_icon_dashboard'],
...
def load_data() -> pd.DataFrame:
    ...
    try:
        ...
        logger.error("Arquivo de dados não encontrado: %s", DATA_FILE)
        raise

    # Aplicar feature engineering usando o método do pipeline
    pipeline = ObesityPredictor()
    df = pipeline.apply_feature_engineering(df)

    # Adicionar coluna de categoria de IMC em português
    def categorize_bmi_pt(bmi: float) -> str:
    ...
    df['BMI_Category_PT'] = df['BMI'].apply(categorize_bmi_pt)
...
def generate_ai_insights(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    Returns:
        Lista de dicionários com insights gerados.
    """
    # Trabalhar com cópia para não modificar dados em cache
    df = df.copy()
    insights = []
...
def create_obesity_distribution_charts(df: pd.DataFrame) -> tuple[go.Figure, go.Figure]:
    """
    Cria gráficos de distribuição de obesidade.

    Args:
        df: DataFrame com os dados.

    Returns:
        Tupla com gráfico de pizza e gráfico de barras.
    """
    obesity_counts = df['Obesity_PT'].value_counts()
    colors_list = [COLORS_PT.get(nivel, '#999999') for nivel in obesity_counts.index]
...
def main() -> None:
    ...
    st.markdown(
        ...
        '<h1><span class="icon">📊</span> Dashboard Analítico</h1>'
        '<p class="subtitle">Estudo sobre Obesidade • Insights para Equipe Médica</p>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    with col1:
        st.metric("Total de Pacientes", f"{len(df):,}")
    ...
    fig_pie, fig_bar = create_obesity_distribution_charts(df)
    ...
    male_obesity_rate = (
        df[df['Gender_PT'] == 'Masculino']['Obesity'].str.contains('Obesity', na=False).sum() /
        len(df[df['Gender_PT'] == 'Masculino'])
    ) * 100

    female_obesity_rate = (
        df[df['Gender_PT'] == 'Feminino']['Obesity'].str.contains('Obesity', na=False).sum() /
        len(df[df['Gender_PT'] == 'Feminino'])
    ) * 100
    ...
    with col1:
        ...
        for nivel in gender_obesity.columns:
            fig_gender.add_trace(go.Bar(
                ...
                marker_color=COLORS_PT.get(nivel, '#999999'),
    ...
    with col2:
        ...
        fig_metrics.update_layout(
            ...
            plot_bgcolor='rgba(0,0,0,0)',
            paper_bgcolor='rgba(0,0,0,0)',
            yaxis=dict(range=[0, max_metric_value * 1.25])
        )

        st.plotly_chart(fig_metrics, use_container_width=True)

    # Gráficos de pizza por gênero
    ...
    with col1:
        ...
        colors_male = [COLORS_PT.get(nivel, '#999999') for nivel in male_obesity.index]
    ...
    with col2:
        ...
        colors_female = [COLORS_PT.get(nivel, '#999999') for nivel in female_obesity.index]
    ...
    with col1:
        # Histórico familiar - trabalhar com cópia
        df_temp = df.copy()
        family_obesity = pd.crosstab(df_temp['family_history'], df_temp['Obesity_PT'], normalize='index') * 100
    ...
    st.info(
        "**Como ler estes gráficos:** Cada barra mostra a taxa de obesidade para diferentes "
        "níveis de atividade física ou consumo de vegetais. Quanto maior a barra vermelha/laranja, "
        "maior o risco de obesidade."
    )

    col1, col2 = st.columns(2)

    with col1:
        # Criar categorias de atividade física - usar cópia
        df_faf = df.copy()
        df_faf['FAF_Category'] = pd.cut(
            df_faf['FAF'].round(),
            bins=[-0.5, 0.5, 1.5, 2.5, 3.5],
            labels=['Nenhuma', '1-2x/semana', '2-3x/semana', '4-5x/semana']
        )

        faf_analysis = df_faf.groupby('FAF_Category').agg({
        ...
        fig_faf.update_layout(
            ...
            plot_bgcolor='rgba(0,0,0,0)',
            paper_bgcolor='rgba(0,0,0,0)',
            yaxis=dict(range=[0, max_faf_value * 1.3])
        )
        ...
        st.success(
            f"**Insight:** Pessoas com '{max_faf}' atividade física têm {diff_faf:.1f} pontos "
            f"percentuais a mais de risco de obesidade comparado a '{min_faf}'."
        )

    with col2:
        # Criar categorias de consumo de vegetais - usar cópia
        df_fcvc = df.copy()
        df_fcvc['FCVC_Category'] = pd.cut(
            df_fcvc['FCVC'].round(),
            bins=[0.5, 1.5, 2.5, 3.5],
            labels=['Baixo (1)', 'Moderado (2)', 'Alto (3)']
        )

        fcvc_analysis = df_fcvc.groupby('FCVC_Category').agg({
            'Obesity': lambda x: (x.str.contains('Obesity', na=False).sum() / len(x) * 100)
        }).round(1)
        ...
        fig_fcvc.update_layout(
            ...
            plot_bgcolor='rgba(0,0,0,0)',
            paper_bgcolor='rgba(0,0,0,0)',
            yaxis=dict(range=[0, max_fcvc_value * 1.3])
        )
        ...
        st.success(
            f"**Insight:** Consumo '{max_fcvc}' de vegetais está associado a {diff_fcvc:.1f} pontos "
            f"percentuais a mais de risco de obesidade comparado a '{min_fcvc}'."
        )
    ...
    for insight in ai_insights:
        with st.expander(insight['title'], expanded=True):
            ...
            fig_insight.update_layout(
                ...
                yaxis=dict(range=[0, max_insight_value * 1.3]),
                showlegend=False
            )
    ...
    recommendations = [
        {
            ...
            'content': (
                'Implementar avaliação de risco de obesidade em consultas de rotina, '
                'especialmente para pacientes com histórico familiar. '
                'Criar protocolo de triagem rápida usando este sistema preditivo.'
            )
        },
        {
            'icon': '📚',
            ...
            'content': (
                'Desenvolver programas focados em educação nutricional e promoção de atividade física. '
                'Priorizar pacientes identificados com múltiplos fatores de risco.'
            )
        },
        {
            'icon': '📊',
            ...
            'content': (
                'Acompanhar pacientes com múltiplos fatores de risco '
                '(histórico familiar + baixa atividade física + consumo de alimentos calóricos). '
                'Estabelecer check-ups trimestrais.'
            )
        },
        {
            'icon': '⚡',
            ...
            'content': (
                'Identificar e intervir em pacientes com sobrepeso antes que progridam para obesidade. '
                'A intervenção precoce tem maior taxa de sucesso e menor custo.'
            )
        },
        {
            'icon': '👥',
            ...
            'content': (
                'Envolver nutricionistas, educadores físicos e psicólogos no tratamento de pacientes com obesidade. '
                'Criar equipe multidisciplinar dedicada.'
            )
        }
    ]
